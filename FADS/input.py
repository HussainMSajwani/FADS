...
def get_dset(path, generate_ct=True):
    ...
    ohe = OneHotEncoder(categories=[[False, True]])
    #ohe.fit_transform(Y.reshape(-1, 1)).toarray()
    #ohe.fit_transform(Y.reshape(-1, 1)).toarray()
    X_train = load(f"{path}/X_train.npy", allow_pickle=True)
    y_train = ohe.fit_transform(load(f"{path}/y_train.npy", allow_pickle=True).reshape(-1, 1)).toarray()

    X_test = load(f"{path}/X_test.npy", allow_pickle=True)
    y_test = ohe.fit_transform(load(f"{path}/y_test.npy", allow_pickle=True).reshape(-1, 1)).toarray()
